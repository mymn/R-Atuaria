...
importardados=function(){
  ...
  planilha<-read_excel("local da planilha")
  nfluxos=length(planilha[datacompra])
...
calculocarteira=function(curva){
  ...
  curva()
  for(i in 1:nfluxos){
      carteira[1,i]=Tojulian(planilha[datavencimento])-Tojulian(planilha[datacompra])*252/365
      carteira[3,i]=Tojulian(planilha[datavencimento])-Tojulian(hoje)*252/365
      carteira[2,i]=planilha[quantidade]*1000
      fluxo[i,1]=planilha[Tojulian(datavencimento)]
